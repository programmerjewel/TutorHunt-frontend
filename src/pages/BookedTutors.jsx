...
const BookedTutors = () => {
  ...
  const fetchBookedTutors = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:4000/booked-tutors?email=${user.email}`);
  ...
  return (
    <main>
      ...
      {
        tutor.length === 0 ? (<div className="text-center py-10">
          <p className="text-xl">You haven't booked any tutor yet.</p>
        </div>) : (<div className="grid md:grid-cols-4 grid-cols-2 gap-6">
        {
